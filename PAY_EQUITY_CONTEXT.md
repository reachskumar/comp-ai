...
- ✅ Phase 4 — Prevent half shipped — band drift detector + unified pre-decision equity check (covers 4.3 promotion slate, 4.6 in-cycle warning, 4.7 pre-offer guardrail via one endpoint)
- ✅ Phase 3.7 (scheduled delivery) + 6.4 (CHRO daily digest) shipped — PEReportSubscription table + BullMQ hourly cron + email/Slack dispatch
- ✅ Phase 5.5 (external auditor portal) shipped — PEShareToken table + public /pe-share/:token route with expiry + revocation + access count
- ⬜ Phase 6.1 employee personal equity statement, 6.2 pay range publication, 3.6 comp committee deck, 2.4 multi-quarter plan generator, 2.6 remediation letters hook (chunk 3 of 100% push)
...
**What's next (chunk 3 of 3 in the 100% push):**

1. 2.4 Multi-quarter plan generator
2. 2.6 Remediation letters hook into Letters batch infra
3. 3.6 Comp committee deck generator
4. 6.1 Employee personal equity statement
5. 6.2 Pay range publication module (CA / NY / CO / EU)
6. 6.1 Employee personal equity statement
7. 6.2 Pay range publication module (CA / NY / CO / EU)
8. Phase 4 Prevent half (4.3 promotion slate, 4.4 pay band drift, 4.6 in-cycle warning, 4.7 pre-offer guardrail) — needs comp cycle / offer-flow hooks
9. Phase 5.5 external auditor portal — share-token + read-only UI
10. Phase 2.4 / 2.6 / 3.6 / 3.7 — multi-quarter plan, letters hook, comp committee deck, scheduled delivery
...
| 3.7 | Scheduled delivery (annual/quarterly)   | ✅     | `POST /pay-equity/subscriptions` (cadence: daily/weekly/monthly/quarterly + recipients[]). New `PEReportSubscription` table; BullMQ hourly cron (`PEDistributionProcessor`) calls `runDueSubscriptions()` which generates the artifact and emails recipients. Errors → `lastError`; nextRunAt rescheduled either way. |
...
### Phase 4 — Predict & Prevent _(Predict half shipped 2026-04-28; Prevent half shipped 2026-04-29)_
...
| 4.3 | Promotion slate equity check              | ✅     | Folded into the unified `POST /pay-equity/preview-change` with `kind: 'promotion'`. Caller passes a list of promotions, gets verdict + flagged employees. Comp cycle module can call inline before applying a slate.                                                                                                              |
| 4.4 | Pay band drift detector                   | ✅     | `GET /pay-equity/band-drift`. Compares weighted-mean compa-ratio across the last 6 runs. Verdict: bands_outpacing / salaries_outpacing / stable / insufficient_history. No new schema — uses the run envelopes' compaRatios field.                                                                                                |
...
| 4.6 | In-cycle warning at manager workspace     | ✅     | Same `POST /pay-equity/preview-change` endpoint with `kind: 'salary_change'`. Returns verdict (safe/warn/block) + flagged employees (anyone whose post-change CR drops below 0.85). Comp cycle module just needs to call this before save; the equity logic itself is shipped.                                                    |
| 4.7 | Real-time pre-offer guardrail             | ✅     | Same `POST /pay-equity/preview-change` endpoint with `kind: 'new_hire'`. Recruiter offer flow can call this with the candidate's level/dimension/group/salary to get an equity verdict before the offer goes out. Independent of any specific offer module.                                                                       |
...
| 5.5 | External auditor read-only portal         | ✅     | New `PEShareToken` table (sha256-base64url tokens, 30-day default expiry, revocable). Public route `GET /api/v1/pe-share/:token` resolves the token and returns the auditor or defensibility PDF — no JWT required, the token is the credential. accessCount + lastAccessedAt tracked for audit. UI: ShareTokensCard on Reports tab mints + lists + revokes tokens. |
...
| 6.4 | CHRO daily digest (Slack + email)  | ✅     | Same `PEReportSubscription` infra with `reportType='digest'`. Composes a 4-line summary from `getOverview()` (worst gap + cohort, significant count, sample size + confidence, methodology, optional Δ vs previous run) and dispatches via email to recipients[] and/or to the configured Slack incoming-webhook URL. Audit: `PAY_EQUITY_DIGEST_SENT`.                                                |
...
| 4.4         | `GET /pay-equity/band-drift` — pay band drift detector (weighted-mean compa-ratio across last 6 runs)                                                                                                        | ✅     | TBD    | Read-only; no LLM. Verdict: bands_outpacing / salaries_outpacing / stable / insufficient_history. Threshold ±2% relative                                                                                             |
| 4.3+4.6+4.7 | `POST /pay-equity/preview-change` — unified pre-decision equity check (covers promotion slate, in-cycle salary change, pre-offer)                                                                            | ✅     | TBD    | Deterministic, no LLM. Returns baselineGap + projectedDelta + projectedGap + verdict (safe/warn/block) + flagged employees (CR < 0.85 floor). Designed to be called inline from comp-cycles or recruiter offer flows |
| 4.x         | BandDriftCard + PreviewChangeCard UI on the Prevent tab                                                                                                                                                      | ✅     | TBD    | Drift sparkline + verdict badge; staging form for changes with kind selector; result block with verdict-tinted background and flagged-employee list                                                                  |
| 4.x         | Web hooks usePayEquityBandDrift + usePreviewChangeMutation + types                                                                                                                                           | ✅     | TBD    | apps/web/src/hooks/use-pay-equity.ts                                                                                                                                                                                 |
| 4.x         | 7 new service tests (band drift no-data / outpacing / insufficient_history; previewChange ref-group widens / minority narrows / CR-floor flag / no-baseline)                                                 | ✅     | TBD    | 93 total green (65 service + 28 eval)                                                                                                                                                                                |
| 3.7+6.4+5.5 | New `PEDistributionService` (subscriptions + share tokens) + `PEDistributionProcessor` (BullMQ hourly cron)                                                                                                  | ✅     | TBD    | Routed through `LettersModule` for email. Slack via incoming-webhook fetch. Migration `20260429140000_add_pe_subscriptions_and_share_tokens` adds 2 new tables.                                                      |
| 3.7+6.4     | `POST /pay-equity/subscriptions` + `GET /pay-equity/subscriptions` + `PATCH /pay-equity/subscriptions/:id` (delete) + `POST /pay-equity/subscriptions/run-due` (manual cron tick)                            | ✅     | TBD    | Audit actions: `PAY_EQUITY_SUBSCRIPTION_CREATED` / `_DELETED` / `_DIGEST_SENT` / `_REPORT_DELIVERED`                                                                                                                 |
| 5.5         | `POST /pay-equity/share-tokens` + `GET /pay-equity/share-tokens` + `PATCH /pay-equity/share-tokens/:id/revoke` + public `GET /api/v1/pe-share/:token`                                                        | ✅     | TBD    | Public route is unguarded — token IS the credential. Throttled 60/min. Returns auditor or defensibility PDF based on token scope                                                                                     |
| 3.7+6.4+5.5 | SubscriptionsCard + ShareTokensCard UI on the Reports tab                                                                                                                                                    | ✅     | TBD    | Subscription form (type/cadence/recipients/slack), live list with next/last/lastError, share-token form (scope/expiry days), live token list with status badge + access count + revoke button                        |
| 3.7+6.4+5.5 | Web hooks: usePESubscriptions, useCreate/DeleteSubscriptionMutation, usePEShareTokens, useCreate/RevokeShareTokenMutation                                                                                    | ✅     | TBD    | apps/web/src/hooks/use-pay-equity.ts                                                                                                                                                                                 |
| 3.7+6.4+5.5 | 9 new service tests in `pe-distribution.test.ts` (sub create/audit/reject paths; cron dispatch digest + email + audit; cron error path with lastError; share token create / scope reject / revoke / expired) | ✅     | TBD    | 102 total green (74 service + 28 eval)                                                                                                                                                                               |
...
| 2026-04-29 | Phase 4 Prevent: 4.3 + 4.6 + 4.7 collapsed into a single `preview-change` endpoint with `kind` discriminator | All three are "given hypothetical compensation changes, predict the equity impact." The math, the verdict, and the flagged-employee logic are identical — only the input shape varies (promotion slate vs salary change vs new hire). One endpoint with a discriminated union beats three near-identical endpoints. Comp cycle / offer flow integrations call the same path with different `kind` values. |
| 2026-04-29 | Phase 4 Prevent half is deterministic-only (no LLM) | Pre-decision guardrails need to feel snappy — sub-100ms inline call from /comp-cycles or a recruiter offer screen. An LLM call adds 1-3s latency and a hallucination surface that adds nothing here (the answer is just "would this change widen the gap?"). The math is simple enough that the LLM would just be paraphrasing it. |
| 2026-04-29 | Phase 4.4 (band drift) reads existing run envelopes; no SalaryBandSnapshot table introduced | The signal we want — "are salaries lagging the bands over time?" — is already encoded in `compaRatio` (which is salary/band-mid). Tracking band history separately would be more precise but requires a migration + backfill the bible doesn't currently call for. CR-based drift is honest about what we have today and is reproducible from the immutable envelopes. |
| 2026-04-29 | Phase 3.7 + 6.4 share infra: one `PEReportSubscription` table with `reportType` discriminator (`'digest'` for the CHRO summary, otherwise a renderer report type) | Same row shape, same cron, same audit action prefix. Two tables would have introduced an artificial split between "report" and "digest" subscriptions when the only behavioral difference is what the dispatcher composes. The discriminator keeps the cron loop simple and lets us add new report types without a schema change. |
| 2026-04-29 | Phase 5.5 share-token portal is unguarded and unauthenticated — the token IS the credential | An auditor doesn't have a tenant account. Standard practice for read-only links: random base64url token (24 bytes), expires_at + revoked_at, throttled 60/min. The token is generated server-side, embedded in a URL the customer sends to the auditor, and decremented from active → expired/revoked atomically on access. Cross-tenant correlation is impossible because the token is a random opaque string with no tenant prefix. |
| 2026-04-29 | Phase 3.7 cron uses BullMQ repeatable jobs scheduled at module init, not k8s cron | We already have BullMQ wired for letters batch. Re-using means: no new infra, distributed lock semantics already correct, retry/backoff already configured. The trade-off — coupling availability of the cron to having Redis up — is acceptable because Redis is already required by the rest of the API. |
| 2026-04-29 | LetterEmailService extended with optional `attachments` field; not a fork into a new service | Phase 3.7 needs to attach a generated PDF/CSV to an email. The Letters service already wraps nodemailer, knows how to authenticate, and respects EmailNotConfiguredError. Adding an optional field is a 4-line additive change; forking into a new email service would be feature-padding plus drift between two near-identical wrappers. |
...
| 2026-04-29 | Phase 4 Prevent half shipped (chunk 1 of 100% push): `GET /pay-equity/band-drift` (4.4) compares weighted-mean compa-ratio across last 6 runs to detect bands_outpacing / salaries_outpacing / stable. `POST /pay-equity/preview-change` (4.3 + 4.6 + 4.7 collapsed into one) takes hypothetical changes (promotion / salary_change / new_hire) and returns deterministic verdict (safe/warn/block) + flagged-employee list (anyone whose post-change CR drops below 0.85). No LLM — designed for sub-100ms inline calls from comp cycles or a recruiter offer flow. BandDriftCard + PreviewChangeCard added to the Prevent tab. 7 new service tests, 93 total green (65 service + 28 eval).                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Claude                         |
| 2026-04-29 | Phase 3.7 + 6.4 + 5.5 shipped (chunk 2 of 100% push): Two new tables (`PEReportSubscription`, `PEShareToken`) via migration `20260429140000_add_pe_subscriptions_and_share_tokens`. New `PEDistributionService` with subscription CRUD + cron scanner + Slack webhook + share-token CRUD. `PEDistributionProcessor` (BullMQ repeatable job) ticks hourly, dispatches due subscriptions, reschedules nextRunAt + records lastError. CHRO digest composes a 4-line summary from `getOverview()`, sends via email + optional Slack incoming-webhook. Public unauthenticated route `GET /api/v1/pe-share/:token` (60/min throttle) redeems share tokens for the auditor or defensibility PDF — token IS the credential, no tenant account needed. SubscriptionsCard + ShareTokensCard UI on the Reports tab (mint, list, revoke). Letters' `LetterEmailService` extended with optional attachments. 9 new service tests in `pe-distribution.test.ts`. 102 total green (74 service + 28 eval).                                                                                                                                                                        | Claude                         |
