...
export class ReportsService implements ReportBuilderDbAdapter {
  ...
  async *streamGenerate(
    ...
    const threadId = conversationId ?? `report-${tenantId}-${userId}-${Date.now()}`;
  ...
  async getReport(tenantId: string, id: string) {
    const report = await this.db.forTenant(tenantId, (tx) =>
  ...
  private async queryCompRecommendations(
    tenantId: string,
    params: {
      filters?: Record<string, unknown>;
      limit?: number;
