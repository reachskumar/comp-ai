...
import type { EmployeeData, RuleCondition, RuleAction, RuleType } from '@compensation/shared';
  ...
  if (!invert) {
    switch (operator) {
      case 'eq':
        ...
        break;
      case 'neq':
        rec[field] = typeof value === 'string' ? `not-${value}` : -9999;
        break;
      case 'gt':
      case 'gte':
        rec[field] = Number(value) + 1;
        break;
      case 'lt':
      case 'lte':
        rec[field] = Number(value) - 1;
        break;
      case 'in':
        if (Array.isArray(value) && value.length > 0) rec[field] = value[0];
        break;
      case 'notIn':
        rec[field] = '__not_in_list__';
        break;
      case 'between':
        if (Array.isArray(value) && value.length === 2)
          rec[field] = (Number(value[0]) + Number(value[1])) / 2;
        break;
      case 'contains':
        rec[field] = `prefix-${String(value)}-suffix`;
        break;
      case 'startsWith':
        rec[field] = `${String(value)}-rest`;
        break;
      case 'matches':
        rec[field] = String(value).replace(/[.*+?^${}()|[\]\\]/g, '');
        break;
    }
  } else {
    switch (operator) {
      case 'eq':
        rec[field] = typeof value === 'string' ? `wrong-${value}` : -99999;
        break;
      case 'neq':
        rec[field] = value;
        break;
      ...
      case 'notIn':
        ...
        break;
      case 'between':
        if (Array.isArray(value) && value.length === 2) rec[field] = Number(value[1]) + 1000;
        break;
      default:
        rec[field] = '__invalid__';
        break;
    }
  }
}
...
function expectedRangeFromActions(
  ...
  for (const action of actions) {
    ...
    switch (action.type) {
      case 'setMerit':
        result.meritRange = { min: computed * 0.9, max: computed * 1.1 };
        break;
      case 'setBonus':
        result.bonusRange = { min: computed * 0.9, max: computed * 1.1 };
        break;
      case 'setLTI':
        result.ltiRange = { min: computed * 0.9, max: computed * 1.1 };
        break;
    }
  }
  return result;
...
export class TestGeneratorService {
  ...
  async generateTestCases(tenantId: string, ruleSetId: string): Promise<TestCaseData[]> {
    ...
    for (const rule of dbRuleSet.rules) {
      ...
      testCases.push(
        await this.saveTestCase(ruleSetId, {
          name: `${rule.name} — should apply when all conditions match`,
          input: matchEmp,
          expectedOutput: {
            shouldApplyRules: [rule.id],
            shouldSkipRules: [],
            blocked: isBlock,
            ...ranges,
          },
      ...
      if (ruleType === 'CAP' || ruleType === 'FLOOR') {
        ...
        const capAmt = Number(
          actions[0]?.params['amount'] ?? actions[0]?.params['percentage'] ?? 0,
        );
        if (capAmt > 0) {
