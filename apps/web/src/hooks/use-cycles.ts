...
export function useCycleList(page = 1, limit = 20) {
  return useQuery<CycleListResponse>({
    ...
    queryFn: () =>
      apiClient.fetch<CycleListResponse>(
        `/api/v1/cycles?page=${page}&limit=${limit}`
      ),
  });
}
...
export function useCycleSummary(id: string | null) {
  return useQuery<CycleSummary>({
    ...
    queryFn: () =>
      apiClient.fetch<CycleSummary>(`/api/v1/cycles/${id}/summary`),
    enabled: !!id,
  });
}
...
export function useCycleAlerts(id: string | null) {
  return useQuery<CycleAlert[]>({
    ...
    queryFn: () =>
      apiClient.fetch<CycleAlert[]>(`/api/v1/cycles/${id}/monitors/alerts`),
    enabled: !!id,
  });
}
...
export function useRecommendations(
  ...
  if (filters?.outlier !== undefined)
    params.set("outlier", String(filters.outlier));
...
export function usePendingApprovals(cycleId: string | null) {
  return useQuery<PendingApproval[]>({
    ...
    queryFn: () =>
      apiClient.fetch<PendingApproval[]>(
        `/api/v1/cycles/${cycleId}/approvals/pending`
      ),
    enabled: !!cycleId,
  });
}
...
export function useMonitorSummary(cycleId: string | null) {
  return useQuery<MonitorSummary>({
    ...
    queryFn: () =>
      apiClient.fetch<MonitorSummary>(
        `/api/v1/cycles/${cycleId}/monitors/summary`
      ),
    enabled: !!cycleId,
  });
}
...
export function useTransitionCycleMutation() {
  ...
  return useMutation<
    Cycle,
    Error,
    { cycleId: string; targetStatus: CycleStatus; reason?: string }
  >({
...
export function useCreateCalibrationMutation() {
  ...
  >({
    mutationFn: ({ cycleId, ...body }) =>
      apiClient.fetch<CalibrationSession>(
        `/api/v1/cycles/${cycleId}/calibration`,
